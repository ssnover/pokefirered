...
static const u8 sSecretBaseFacilityClasses[GENDER_COUNT][NUM_SECRET_BASE_CLASSES] =
    {
        [MALE] = {
            ...
            FACILITY_CLASS_YOUNGSTER,
            FACILITY_CLASS_YOUNGSTER},
        [FEMALE] = {FACILITY_CLASS_YOUNGSTER, FACILITY_CLASS_YOUNGSTER, FACILITY_CLASS_YOUNGSTER, FACILITY_CLASS_YOUNGSTER, FACILITY_CLASS_YOUNGSTER},
};
...
u8 GetMonsStateToDoubles(void)
{
    ...
    for (i = 0; i < gPlayerPartyCount; i++)
    {
        ...
        if (GetMonData(&gPlayerParty[i], MON_DATA_HP, NULL) != 0 && GetMonData(&gPlayerParty[i], MON_DATA_SPECIES_OR_EGG, NULL) != SPECIES_NONE && GetMonData(&gPlayerParty[i], MON_DATA_SPECIES_OR_EGG, NULL) != SPECIES_EGG)
            aliveCount++;
    }
...
bool8 PokemonUseItemEffects(struct Pokemon *mon, u16 item, u8 partyIndex, u8 moveIndex, bool8 usedByAI)
{
    ...
    for (cmdIndex = 0; cmdIndex < ITEM_EFFECT_ARG_START; cmdIndex++)
    {
        ...
        case 3:
            ...
            // Cure status
            if ((itemEffect[cmdIndex] & ITEM3_SLEEP) && HealStatusConditions(mon, partyIndex, STATUS1_SLEEP, battleMonId) == 0)
            {
...
bool8 PokemonItemUseNoEffect(struct Pokemon *mon, u16 item, u8 partyIndex, u8 moveIndex)
{
    ...
    for (cmdIndex = 0; cmdIndex < ITEM_EFFECT_ARG_START; cmdIndex++)
    {
        ...
        case 3:
            ...
            // Cure status
            if ((itemEffect[cmdIndex] & ITEM3_SLEEP) && PartyMonHasStatus(mon, partyIndex, STATUS1_SLEEP, battlerId))
                retVal = FALSE;
            ...
            while (curEffect)
            {
                if (curEffect & 1)
                {
                    ...
                    case 4: // ITEM5_PP_MAX
                        ...
                        break;
                    case 5: // ITEM5_FRIENDSHIP_LOW
                        if (GetMonData(mon, MON_DATA_FRIENDSHIP, NULL) < 100 && retVal == FALSE && sp18 == 0)
                            sp18 = itemEffect[idx];
                        idx++;
                        break;
                    case 6: // ITEM5_FRIENDSHIP_MID
                        if (GetMonData(mon, MON_DATA_FRIENDSHIP, NULL) >= 100 && GetMonData(mon, MON_DATA_FRIENDSHIP, NULL) < 200 && retVal == FALSE && sp18 == 0)
                            sp18 = itemEffect[idx];
                        idx++;
                        break;
                    case 7: // ITEM5_FRIENDSHIP_HIGH
                        if (GetMonData(mon, MON_DATA_FRIENDSHIP, NULL) >= 200 && retVal == FALSE && sp18 == 0)
                            sp18 = itemEffect[idx];
                        idx++;
                        break;
                    }
...
{
    if (species == SPECIES_SPINDA && dest != gMonSpritesGfxPtr->sprites[B_POSITION_PLAYER_LEFT] && dest != gMonSpritesGfxPtr->sprites[B_POSITION_PLAYER_RIGHT])
